...
Rollup-Web 不单单只是 Rollup 的浏览器版本，它还对 Worker、 iframe 环境进行了适配，提供了一整套的浏览器特殊 plugins，所以可以实现一体化的文件打包执行。

## Rollup-Web 执行过程

Rollup-Web 的执行过程大体如下图所示：

<Embed src="/doc/zh_cn/Guide/assets/implementation.drawio"></Embed>

## 执行线程的设计

Rollup-Web 提供了直接初始化执行线程的方式。执行线程的任务就是将初始 URL 请求打包线程打包，执行返回的代码，并在执行后进行依赖代码打包请求。

> 执行线程可以有多种宿主环境： 主线程`可以操作 DOM`，Worker 线程`在源代码中创建了 Worker`、Iframe 线程

1. 执行环境的预准备：根据执行环境的不同，我们将调节 Worker 和 Iframe 将一些全局属性定义为正确的参数。

2. 代码执行环境引入了 systemjs 进行模块操作，我们对其进行了一系列定制，使得它的模块请求转接到打包线程。

3. 请求代码打包：将相对路径的 URL 转化为绝对路径，并交由打包线程进行操作。

4. 执行打包代码：根据执行环境的不同，我们需要对打包代码的模块内的全局变量进行复写，以保证正确的 location 等变量，然后进行代码执行。
...
> 所以，Rollup-Web 是支持部分的 Rollup 插件的。（如 replace、json、commonjs 等）
...
我们在 Rollup-Web 系统中提供了缓存机制保证部署展示时速度超级快。缓存机制的主要底层为 IndexDB，我们使用了 localforage 这个库进行便捷的操作。在其上，我们制作了一个插件管理系统，保证我们的缓存的可扩展性。
...
对于原本应该处于 node_modules 中对应的模块，这些年已经有 CDN 在进行打包操作了，所以我们需要的是直接更改模块裸导入为 CDN 提供的 URL！这种解决方案我们称之为 CDN Module。
...
3. [ESM.sh](https://esm.sh/)：底层采用 ESBuild 进行打包，所以对于一些 Nodejs 的功能会做补充，使得我们可以使用。(babel 的一系列插件就是通过这个 CDN 导入的)
